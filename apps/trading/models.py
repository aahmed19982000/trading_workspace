...
class TradingAccount(models.Model):
    ...
    equity = models.DecimalField(
        "حقوق الملكية", max_digits=15, decimal_places=2, default=0
    )
...
class RiskSettings(models.Model):
    ...
    allow_weekend_holding = models.BooleanField(
        "السماح بالاحتفاظ لعطلة نهاية الأسبوع", default=False
    )
    created_at = models.DateTimeField("تاريخ الإنشاء", auto_now_add=True)
    updated_at = models.DateTimeField("تاريخ التحديث", auto_now=True)
...
class Trade(models.Model):
    ...
    exit_price = models.DecimalField(
        "سعر الخروج", max_digits=15, decimal_places=5, null=True, blank=True
    )
    stop_loss = models.DecimalField(
        "وقف الخسارة", max_digits=15, decimal_places=5, null=True, blank=True
    )
    take_profit = models.DecimalField(
        "جني الأرباح", max_digits=15, decimal_places=5, null=True, blank=True
    )
    pnl = models.DecimalField(
        "الربح والخسارة", max_digits=15, decimal_places=2, default=0
    )
    status = models.CharField(
        "الحالة", max_length=20, choices=TRADE_STATUSES, default="pending"
    )
    external_ticket = models.CharField(
        "رقم العملية الخارجي", max_length=100, blank=True
    )
...
class PerformanceReport(models.Model):
    ...
    win_rate = models.DecimalField(
        "نسبة الربح", max_digits=5, decimal_places=2, default=0
    )
    net_profit = models.DecimalField(
        "صافي الربح", max_digits=15, decimal_places=2, default=0
    )
    max_drawdown = models.DecimalField(
        "أقصى تراجع", max_digits=15, decimal_places=2, default=0
    )
